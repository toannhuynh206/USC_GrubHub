...
<head>
		...
		<body onLoad="test();">		
			...
			<div class = "username" style="position: absolute; top: 70px; left: 330px; ">
			<% String name = "";
...
<h3 style="margin-left: 75px;"> Results for "<%= request.getParameter("name") %>" </h3>
			...
			<c:forEach items="${restaurants}" var="r">
				...
				<c:url value="details.jsp" var="myURL">
					...
					<c:param name="categories" value="${r.getCatString()}" />
				...
				<div class="media">
					...
					<a href="${myURL}"> <h5 style="margin-top: 40px; color: black;"> <c:out value="${r.getName()}" /> <br> </h5></a> 
					<div style="font-size: 15px; line-height: 30px;">	
						Price: <c:out value="${r.getPrice()}" /> <br>
						Review Count: <c:out value="${r.getReviewcount()}" /> <br>
						Rating: <span class= "rstars"><c:out value="${r.getRating()}" /> </span><br>
						<a target="_blank" href="${r.getUrl()}"> Yelp Link </a> <br> 
						</div>
					</div>
				</div>
				<hr style="margin: 10px 75px;">
